pub mod day_one {
    ...
    fn get_sorted_co_ords(co_ords: &mut CoOrds) -> &mut CoOrds {
        let data =
            fs::read_to_string("src/day_one/day_one_input.txt").expect("Unable to read file");

        let new_co_cords = data.lines().fold(co_ords, |acc, line| {
            let mut line_split = line.split_whitespace();
            acc.one.append(&mut vec![line_split
                .next()
                .unwrap()
                .trim()
                .parse::<i32>()
                .unwrap()]);
            acc.two.append(&mut vec![line_split
                .next()
                .unwrap()
                .trim()
                .parse::<i32>()
    ...
    pub fn part_one() -> i32 {
        ...
        let result = sorted_co_ords
            .one
            .iter()
            .zip(sorted_co_ords.two.clone())
            .fold(0, |acc, (one, two)| {
                let x = one - two;
                x.abs() + acc
            });
    ...
    pub fn part_two() -> i32 {
        ...
        let sorted_co_ords = get_sorted_co_ords(&mut co_ords);
        let similarity_score: Vec<(i32, i32)> = sorted_co_ords
            .one
            .iter()
            .map(|&x| {
